...
public:
  ...
  // we need two for the (rare) case that there is two pawns beside a double
  // pushed pawn
  Move en_passant[2];
  Move castle_moves[4];
  ...
  inline Color flipColor(const Color &color) const {
  ...
  bitboard getAllPieces(Color color) const {
    return color == WHITE ? piece_BB[WHITE] : piece_BB[BLACK];
  }
  ...
  bitboard getPawns(Color color) const {
    return piece_BB[PAWN] & piece_BB[color];
  }
  bitboard getKnights(Color color) const {
    return piece_BB[KNIGHT] & piece_BB[color];
  }
  bitboard getBishops(Color color) const {
    return piece_BB[BISHOP] & piece_BB[color];
  }
  bitboard getRooks(Color color) const {
    return piece_BB[ROOK] & piece_BB[color];
  }
  bitboard getQueens(Color color) const {
    return piece_BB[QUEEN] & piece_BB[color];
  }
  bitboard getKings(Color color) const {
    return piece_BB[KING] & piece_BB[color];
  }
  ...
  bool checkIntegrity();
  void checkEnPassant(const square &from, const square &to, const Piece &piece,
                      const Color &color);
  ...
  bool setupMoves(std::vector<std::string> moves);
