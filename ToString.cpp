...
// Iterates through the bits of a bitboard and puts in the outputstring if there
// is a match
...
void ToString::printDebug(Board &board) {
  ...
  std::cout << std::endl;
  std::cout << "En passant: " << board.en_passant[0] << " "
            << board.en_passant[1] << std::endl;
...
void ToString::printMove(Move m) {
  std::cout << "From: " << extractFrom(m) << " To: " << extractTo(m)
            << " Piece: " << extractPiece(m)
            << " Capture: " << extractCapture(m)
            << " Promotion: " << extractPromotion(m) << " EP: " << extractEP(m)
            << "Castle: " << extractMovetype(m) << " Move: " << m << std::endl;
}
